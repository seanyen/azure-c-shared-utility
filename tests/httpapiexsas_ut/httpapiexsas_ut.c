...
static void setupSAS_Create_happy_path_provide_key(bool useSasKey, bool allocateKeyName)
{
    ...
    STRICT_EXPECTED_CALL(STRING_c_str(IGNORED_ARG)).SetReturn(useSasKey ? TEST_SAS_KEY : TEST_KEY);
    STRICT_EXPECTED_CALL(STRING_c_str(IGNORED_ARG)).SetReturn(TEST_URI_RESOURCE);
    if (allocateKeyName)
    {
        STRICT_EXPECTED_CALL(STRING_c_str(IGNORED_ARG)).SetReturn(TEST_KEY_NAME);
    }
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_ARG));
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_ARG, IGNORED_ARG));
    STRICT_EXPECTED_CALL(mallocAndStrcpy_s(IGNORED_ARG, IGNORED_ARG));
...
TEST_FUNCTION(HTTPAPIEX_SAS_Create_malloc_state_fails)
{
    ...
    STRICT_EXPECTED_CALL(STRING_c_str(IGNORED_ARG)).SetReturn(TEST_KEY);
    STRICT_EXPECTED_CALL(STRING_c_str(IGNORED_ARG)).SetReturn(TEST_URI_RESOURCE);
    STRICT_EXPECTED_CALL(STRING_c_str(IGNORED_ARG)).SetReturn(TEST_KEY_NAME);
...
TEST_FUNCTION(HTTPAPIEX_SAS_invoke_executerequest_sas_is_provided_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(STRING_c_str(IGNORED_ARG)).SetReturn(TEST_CHAR_ARRAY);
    STRICT_EXPECTED_CALL(HTTPHeaders_ReplaceHeaderNameValuePair(TEST_REQUEST_HTTP_HEADERS_HANDLE, "Authorization", IGNORED_ARG)).SetReturn(HTTP_HEADERS_ERROR);
...
TEST_FUNCTION(HTTPAPIEX_SAS_invoke_executerequest_sastoken_create_returns_null_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(SASToken_CreateString(IGNORED_ARG, IGNORED_ARG, IGNORED_ARG, TEST_EXPIRY)).SetReturn(NULL);
    STRICT_EXPECTED_CALL(HTTPAPIEX_ExecuteRequest(TEST_HTTPAPIEX_HANDLE, TEST_HTTPAPI_REQUEST_TYPE, TEST_CHAR_ARRAY, TEST_REQUEST_HTTP_HEADERS_HANDLE, TEST_REQUEST_CONTENT, &statusCode, TEST_RESPONSE_HTTP_HEADERS_HANDLE, TEST_RESPONSE_CONTENT)).SetReturn(HTTPAPIEX_OK);
...
TEST_FUNCTION(HTTPAPIEX_SAS_invoke_executerequest_replace_header_name_value_pair_fails_succeeds)
{
    ...
    STRICT_EXPECTED_CALL(get_time(NULL)).SetReturn(3600);
    STRICT_EXPECTED_CALL(get_difftime(3600, 0)).SetReturn(3600);
    STRICT_EXPECTED_CALL(SASToken_CreateString(IGNORED_ARG, IGNORED_ARG, IGNORED_ARG, TEST_EXPIRY));
    STRICT_EXPECTED_CALL(STRING_c_str(IGNORED_ARG)).SetReturn(TEST_CHAR_ARRAY);
    STRICT_EXPECTED_CALL(HTTPHeaders_ReplaceHeaderNameValuePair(TEST_REQUEST_HTTP_HEADERS_HANDLE, "Authorization", IGNORED_ARG)).SetReturn(HTTP_HEADERS_ERROR);
    STRICT_EXPECTED_CALL(STRING_delete(IGNORED_ARG));
    STRICT_EXPECTED_CALL(HTTPAPIEX_ExecuteRequest(TEST_HTTPAPIEX_HANDLE, TEST_HTTPAPI_REQUEST_TYPE, TEST_CHAR_ARRAY, TEST_REQUEST_HTTP_HEADERS_HANDLE, TEST_REQUEST_CONTENT, &statusCode, TEST_RESPONSE_HTTP_HEADERS_HANDLE, TEST_RESPONSE_CONTENT)).SetReturn(HTTPAPIEX_OK);
