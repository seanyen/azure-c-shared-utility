...
BEGIN_TEST_SUITE(x509_openssl_unittests)
    ...
    static void setup_load_alias_key_cert_mocks(bool is_rsa_cert)
    {
        if (is_rsa_cert)
        {
            ...
            STRICT_EXPECTED_CALL(SSL_CTX_use_RSAPrivateKey(TEST_SSL_CTX_STRUCTURE, IGNORED_ARG));
    ...
    static void setup_load_certificate_chain_mocks()
    {
        ...
        STRICT_EXPECTED_CALL(PEM_read_bio_X509_AUX(IGNORED_ARG, NULL, NULL, NULL));
        STRICT_EXPECTED_CALL(SSL_CTX_use_certificate(IGNORED_ARG, IGNORED_ARG));
...
#endif
        STRICT_EXPECTED_CALL(PEM_read_bio_X509(IGNORED_ARG, NULL, NULL, NULL));
        // Actual macro name: SSL_CTX_add_extra_chain_cert:
        STRICT_EXPECTED_CALL(SSL_CTX_ctrl(TEST_SSL_CTX_STRUCTURE, SSL_CTRL_EXTRA_CHAIN_CERT, IGNORED_ARG, IGNORED_ARG));
        STRICT_EXPECTED_CALL(PEM_read_bio_X509(IGNORED_ARG, NULL, NULL, NULL))
            .SetReturn(NULL); // Needed because the x509 needs not to be free
        STRICT_EXPECTED_CALL(X509_free(IGNORED_ARG));
    ...
    static void setup_add_credentials_pem_file(bool is_rsa_cert)
    {
        ...
        STRICT_EXPECTED_CALL(PEM_read_bio_PrivateKey(IGNORED_ARG, NULL, NULL, NULL));
#ifndef __APPLE__
        STRICT_EXPECTED_CALL(EVP_PKEY_id(IGNORED_ARG)).SetReturn(is_rsa_cert ? EVP_PKEY_RSA : EVP_PKEY_EC);
#endif
    ...
    TEST_FUNCTION(x509_openssl_add_certificates_1_certificate_happy_path)
    {
        ...
        STRICT_EXPECTED_CALL(BIO_puts(IGNORED_ARG, TEST_CERTIFICATE_1));
        STRICT_EXPECTED_CALL(PEM_read_bio_X509(IGNORED_ARG, IGNORED_ARG, IGNORED_ARG, IGNORED_ARG));
        STRICT_EXPECTED_CALL(X509_STORE_add_cert(IGNORED_ARG, IGNORED_ARG));
        STRICT_EXPECTED_CALL(X509_free(IGNORED_ARG));
