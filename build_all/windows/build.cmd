...
if %MAKE_NUGET_PKG% == yes (
	...
	echo ***Running CMAKE for Win32***
	cmake %build-root% -A Win32 -Drun_unittests:bool=ON -Duse_wsio:bool=ON -Drun_int_tests=ON -Dbuild_csharp_projects:BOOL=OFF -G %VSVERSION%
	if not !ERRORLEVEL!==0 exit /b !ERRORLEVEL!
) else if %build-platform% == ARM (
	echo ***Running CMAKE for ARM***
	cmake %build-root% -A ARM -Drun_unittests:bool=ON -Drun_int_tests=ON -Dbuild_csharp_projects:BOOL=OFF -G %VSVERSION%
	if not !ERRORLEVEL!==0 exit /b !ERRORLEVEL!
) else (
	echo ***Running CMAKE for Win64***
	cmake %build-root% -A x64 -Drun_unittests:bool=ON -Drun_int_tests=ON -Dbuild_csharp_projects:BOOL=OFF -G %VSVERSION%
	if not !ERRORLEVEL!==0 exit /b !ERRORLEVEL!
)
